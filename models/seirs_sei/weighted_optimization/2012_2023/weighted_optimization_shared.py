...
# Load fitted humidity parameters from calibration results if available
_humidity_results_file = os.path.join(_SCRIPT_DIR, "calibration_results.json")
_fitted_H0 = 58.0
_fitted_k = 0.25
_fitted_phi = 0.05
if os.path.exists(_humidity_results_file):
    with open(_humidity_results_file, "r") as _f:
        _humidity_saved = json.load(_f)
    # Use exponential as the default fitted set (best weighted fit)
    _best_method = (
        "exponential"
        if "exponential" in _humidity_saved
        else next(iter(_humidity_saved), None)
    )
    if _best_method:
        _fitted_H0 = _humidity_saved[_best_method]["H0"]
        _fitted_k = _humidity_saved[_best_method]["k"]
        _fitted_phi = _humidity_saved[_best_method]["phi"]
        print(
            f"Loaded fitted humidity params from {_best_method}: H0={_fitted_H0:.4f}, k={_fitted_k:.6f}, phi={_fitted_phi:.6f}"
        )
    else:
        print(
            "Warning: calibration_results.json exists but is empty. Using default humidity params."
        )
else:
    print(
        "Warning: calibration_results.json not found. Using default humidity params (H0=58, k=0.25, phi=0.05)."
    )

# Load fitted human transmission parameters from calibration results if available
_human_results_file = os.path.join(_SCRIPT_DIR, "calibration_results_human.json")
_fitted_tau_H = 10
_fitted_gamma = 1 / 120
_fitted_omega = 1 / 270
if os.path.exists(_human_results_file):
    with open(_human_results_file, "r") as _f:
        _human_saved = json.load(_f)
    _best_human_method = (
        "exponential"
        if "exponential" in _human_saved
        else next(iter(_human_saved), None)
    )
    if _best_human_method:
        _fitted_tau_H = _human_saved[_best_human_method]["tau_H"]
        _fitted_gamma = _human_saved[_best_human_method]["gamma"]
        _fitted_omega = _human_saved[_best_human_method]["omega"]
        print(
            f"Loaded fitted human params from {_best_human_method}: tau_H={_fitted_tau_H:.4f}, gamma={_fitted_gamma:.6f}, omega={_fitted_omega:.6f}"
        )
    else:
        print(
            "Warning: calibration_results_human.json exists but is empty. Using default human params."
        )
else:
    print(
        "Warning: calibration_results_human.json not found. Using default human params (tau_H=10, gamma=1/120, omega=1/270)."
    )
...
def run_with_human_params(tau_H_val, gamma_val, omega_val, M_prime_val=M_prime_warmup):
    """Run warm-up (2012-2016) + main (2017-2023) with custom tau_H, gamma, omega.
    Uses fitted humidity parameters (H0, k, phi) from calibration_results.json."""

    def custom_p_T(Temp, Humid):
        denominator = A * Temp**2 + B * Temp + C
        p_T_temp = np.where(denominator <= 0, 0.0, np.exp(-1 / denominator))
        p_H = _fitted_phi + (1.0 - _fitted_phi) / (
            1.0 + np.exp(-_fitted_k * (Humid - _fitted_H0))
        )
        return p_T_temp * p_H
    ...
    def make_ode_func(N, clim):
        def ode_func(t, z):
            ...
            tau_M_curr = tau_M(T_value)
            b3_m_curr = 1.0 / tau_M_curr if tau_M_curr > 0 else 0.0
            b3_h_curr = 1.0 / tau_H_val if tau_H_val > 0 else 0.0
            l_curr = np.exp(-mu_curr * tau_M_curr)
            b_curr = b_rate(R_value, T_value)
            b3_briere_curr = b3_briere_scaled_plasm(T_curr)
            ...
            dShdt = -foi_h * S_H + omega_val * R_H
            dEhdt = foi_h * S_H - b3_h_curr * E_H
            dIhdt = b3_h_curr * E_H - gamma_val * I_H
            dRhdt = gamma_val * I_H - omega_val * R_H
...
def objective_function_weighted_human(
    params,
    weight_decay="exponential",
    decay_rate=0.005,
    initial_weight=5.0,
    ...
    """
    Weighted objective function for human transmission calibration.

    Parameters:
    -----------
    params : list
        [tau_H, gamma, omega] parameters
    """
    tau_H_val, gamma_val, omega_val = params
    if tau_H_val < 1 or tau_H_val > 60:
        return 1e15
    if gamma_val < 1e-4 or gamma_val > 1.0:
        return 1e15
    if omega_val < 1e-5 or omega_val > 0.1:
        return 1e15

    dates, IH = run_with_human_params(tau_H_val, gamma_val, omega_val)
    if len(dates) == 0:
        return 1e15
...
def objective_function_unweighted_human(params):
    """
    Standard (unweighted) objective function for human transmission calibration.
    """
    tau_H_val, gamma_val, omega_val = params
    if tau_H_val < 1 or tau_H_val > 60:
        return 1e15
...
def run_with_foi(b1_val, b2_val, M_prime_val=M_prime_warmup):
    """Run warm-up (2012-2016) + main (2017-2023) with custom b1, b2.
    Uses fitted humidity (H0, k, phi) and human transmission (tau_H, gamma, omega) parameters."""
    ...
    def make_ode_func(N, clim):
        def ode_func(t, z):
            ...
            tau_M_curr = tau_M(T_value)
            b3_m_curr = 1.0 / tau_M_curr if tau_M_curr > 0 else 0.0
            b3_h_curr = 1.0 / _fitted_tau_H if _fitted_tau_H > 0 else 0.0
            l_curr = np.exp(-mu_curr * tau_M_curr)
            b_curr = b_rate(R_value, T_value)
            b3_briere_curr = b3_briere_scaled_plasm(T_curr)
            ...
            dShdt = -foi_h * S_H + _fitted_omega * R_H
            dEhdt = foi_h * S_H - b3_h_curr * E_H
            dIhdt = b3_h_curr * E_H - _fitted_gamma * I_H
            dRhdt = _fitted_gamma * I_H - _fitted_omega * R_H
...
def objective_function_weighted_foi(
    params,
    weight_decay="exponential",
    decay_rate=0.005,
    initial_weight=5.0,
    ...
    """
    Weighted objective function for FoI calibration.
    ...
    if b1_val < 0.001 or b1_val > 1.0:
        return 1e15
    if b2_val < 0.001 or b2_val > 1.0:
        return 1e15

    dates, IH = run_with_foi(b1_val, b2_val)
    if len(dates) == 0:
        return 1e15
...
def objective_function_unweighted_foi(params):
    """
    Standard (unweighted) objective function for FoI calibration.
    """
    b1_val, b2_val = params
    if b1_val < 0.001 or b1_val > 1.0:
        return 1e15
...
# Load fitted FoI parameters from calibration results if available
_foi_results_file = os.path.join(_SCRIPT_DIR, "calibration_results_foi.json")
_fitted_b1 = 0.04
_fitted_b2 = 0.09
if os.path.exists(_foi_results_file):
    with open(_foi_results_file, "r") as _f:
        _foi_saved = json.load(_f)
    _best_foi_method = (
        "exponential" if "exponential" in _foi_saved else next(iter(_foi_saved), None)
    )
    if _best_foi_method:
        _fitted_b1 = _foi_saved[_best_foi_method]["b1"]
        _fitted_b2 = _foi_saved[_best_foi_method]["b2"]
        print(
            f"Loaded fitted FoI params from {_best_foi_method}: b1={_fitted_b1:.6f}, b2={_fitted_b2:.6f}"
        )
    else:
        print(
            "Warning: calibration_results_foi.json exists but is empty. Using default FoI params."
        )
else:
    print(
        "Warning: calibration_results_foi.json not found. Using default FoI params (b1=0.04, b2=0.09)."
    )


# Load fitted IC parameters from calibration results if available
_ic_results_file = os.path.join(_SCRIPT_DIR, "calibration_results_ic.json")
_fitted_S_H0_frac = 0.60
_fitted_E_H0_frac = 0.05
_fitted_I_M0_ratio = 0.01
if os.path.exists(_ic_results_file):
    with open(_ic_results_file, "r") as _f:
        _ic_saved = json.load(_f)
    _best_ic_method = (
        "exponential" if "exponential" in _ic_saved else next(iter(_ic_saved), None)
    )
    if _best_ic_method:
        _fitted_S_H0_frac = _ic_saved[_best_ic_method]["S_H0_frac"]
        _fitted_E_H0_frac = _ic_saved[_best_ic_method]["E_H0_frac"]
        _fitted_I_M0_ratio = _ic_saved[_best_ic_method]["I_M0_ratio"]
        print(
            f"Loaded fitted IC params from {_best_ic_method}: S_H0_frac={_fitted_S_H0_frac:.4f}, E_H0_frac={_fitted_E_H0_frac:.4f}, I_M0_ratio={_fitted_I_M0_ratio:.6f}"
        )
    else:
        print(
            "Warning: calibration_results_ic.json exists but is empty. Using default IC params."
        )
else:
    print(
        "Warning: calibration_results_ic.json not found. Using default IC params (S_H0_frac=0.60, E_H0_frac=0.05, I_M0_ratio=0.01)."
    )


def run_with_ic(
    S_H0_frac_val, E_H0_frac_val, I_M0_ratio_val, M_prime_val=M_prime_warmup
):
    """Run warm-up (2012-2016) + main (2017-2023) with custom initial condition params.
    Uses fitted humidity (H0, k, phi), human transmission (tau_H, gamma, omega),
    and FoI (b1, b2) parameters."""

    N_wu = N_warmup
    M_0_wu = 10 * N_wu

    S_H0 = round(S_H0_frac_val * N_wu)
    E_H0 = round(E_H0_frac_val * N_wu)
    R_H0 = N_wu - S_H0 - E_H0 - warmup_I_H0
    if R_H0 < 0.01 * N_wu or R_H0 > 0.85 * N_wu:
        return np.array([]), np.array([])
    I_M0 = round(I_M0_ratio_val * M_0_wu)
    E_M0 = round(I_M0 * 2.5)
    S_M0 = M_0_wu - E_M0 - I_M0
    if S_M0 < 0:
        return np.array([]), np.array([])

    state_init = np.array([S_H0, E_H0, warmup_I_H0, R_H0, S_M0, E_M0, I_M0])
    ...
    def make_ode_func(N, clim, M_prime_local):
        def ode_func(t, z):
            ...
            S_H, E_H, I_H, R_H, S_M, E_M, I_M = z
            foi_h = a_curr * _fitted_b2 * (I_M / N)
            foi_m = a_curr * _fitted_b1 * (I_H / N)
    ...
    sol_wu = solve_ivp(
        make_ode_func(N_warmup, climate_full, M_prime_val),
        (0, warmup_end_idx),
        state_init,
        t_eval=np.arange(0, warmup_end_idx, 1),
        method="LSODA",
    ...
    for year in years:
        ...
        num_days = len(year_climate)
        sol = solve_ivp(
            make_ode_func(N, year_climate, M_prime_val),
            [0, num_days],
            current_state,
            t_eval=np.linspace(0, num_days, num_days * 10),
            method="LSODA",
...
def objective_function_weighted_ic(
    params,
    weight_decay="exponential",
    decay_rate=0.005,
    initial_weight=5.0,
    ...
    """
    Weighted objective function for IC calibration.

    Parameters:
    -----------
    params : list
        [S_H0_frac, E_H0_frac, I_M0_ratio] parameters
    """
    S_H0_frac_val, E_H0_frac_val, I_M0_ratio_val = params
    if S_H0_frac_val < 0.01 or S_H0_frac_val > 0.95:
        return 1e15
    if E_H0_frac_val < 0.001 or E_H0_frac_val > 0.50:
        return 1e15
    if I_M0_ratio_val < 0.0001 or I_M0_ratio_val > 0.10:
        return 1e15

    dates, IH = run_with_ic(S_H0_frac_val, E_H0_frac_val, I_M0_ratio_val)
    if len(dates) == 0:
        return 1e15
...
def objective_function_unweighted_ic(params):
    """
    Standard (unweighted) objective function for IC calibration.
    """
    S_H0_frac_val, E_H0_frac_val, I_M0_ratio_val = params
    if S_H0_frac_val < 0.01 or S_H0_frac_val > 0.95:
        return 1e15
...
def mprime_K0_constraint(M_prime_val):
    """Check the K_0 >= M_0 constraint at the start of the main period (2017).

    The initial mosquito population M_0 (2017) must not exceed the carrying
    capacity K_0 implied by M' at the start of the fitting period.
    ...
    M_0 = 10 * round(pop_by_year[2017])
    return M_0 > K_0


def run_with_mprime(M_prime_val):
    """Run warm-up (2012-2016) + main (2017-2023) with custom M_prime.
    Uses fitted humidity (H0, k, phi), human transmission (tau_H, gamma, omega),
    FoI (b1, b2), and IC (S_H0_frac, E_H0_frac, I_M0_ratio) parameters.
    The warm-up uses the SAME M_prime_val so that the 2017 ICs are consistent
    with the carrying capacity being tested."""

    N_wu = N_warmup
    M_0_wu = 10 * N_wu

    S_H0 = round(_fitted_S_H0_frac * N_wu)
    E_H0 = round(_fitted_E_H0_frac * N_wu)
    R_H0 = N_wu - S_H0 - E_H0 - warmup_I_H0
    if R_H0 < 0.01 * N_wu or R_H0 > 0.85 * N_wu:
        return np.array([]), np.array([])
    I_M0 = round(_fitted_I_M0_ratio * M_0_wu)
    E_M0 = round(I_M0 * 2.5)
    S_M0 = M_0_wu - E_M0 - I_M0
    if S_M0 < 0:
        return np.array([]), np.array([])
...
def objective_function_weighted_mprime(
    log_M_prime,
    weight_decay="exponential",
    decay_rate=0.005,
    initial_weight=5.0,
    ...
    """
    Weighted objective function for M' calibration.

    Parameters:
    -----------
    log_M_prime : float
        log of M' (mosquito carrying capacity base)
    """
    M_prime_val = np.exp(log_M_prime)
    if M_prime_val < 1000 or M_prime_val > 1e9:
        return 1e15
    if mprime_K0_constraint(M_prime_val):
        return 1e15

    dates, IH = run_with_mprime(M_prime_val)
    if len(dates) == 0:
        return 1e15
...
def objective_function_unweighted_mprime(log_M_prime):
    """
    Standard (unweighted) objective function for M' calibration.
    """
    M_prime_val = np.exp(log_M_prime)
    if M_prime_val < 1000 or M_prime_val > 1e9:
        return 1e15
