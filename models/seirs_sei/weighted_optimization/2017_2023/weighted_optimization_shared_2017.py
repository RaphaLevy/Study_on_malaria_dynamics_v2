...
# Path to data directory (repo root, four levels up from 2017_2023/)
...
# Load climate data (2016-2024 file, filtered to 2017-2023)
...
# Filter for analysis period (2017-2023)
...
# ---------------------------------------------------------------------------
# 2017 direct start (no warm-up). Fixed carrying capacity M' = 40 x N_2017.
# ---------------------------------------------------------------------------
N_2017 = round(pop_by_year[2017])
M_PRIME_FIXED = 40 * N_2017

# Fixed 2017 initial conditions (ode_models proportions):
#   E_H0 = 5% N, R_H0 = 15% N, I_H0 from notification data,
#   I_M0 = 2% M_0, E_M0 = 3 x I_M0, S_H0/S_M0 as residuals.
...
# Simulation
# ---------------------------------------------------------------------------
def run_with_humidity_2017(H0_val, k_val, phi_val, M_prime_val=M_PRIME_FIXED):
    """Run the simulation starting directly in 2017 (no warm-up) with custom
    H0, k, phi. Uses the fixed 2017 initial conditions and M' = 40 x N_2017.
    Human transmission (tau_H, gamma, omega) and FoI (b1, b2) are kept at the
    default values, consistent with the warm-up humidity calibration."""
    ...
    all_IH = []
    all_dates = []
    current_state = initial_state_2017
    for year in years:
...
def objective_function_weighted_2017(
    params,
    weight_decay="exponential",
    decay_rate=0.005,
    initial_weight=5.0,
    ...
    """
    Weighted objective function for the 2017 direct-start humidity calibration.
    ...
    dates, IH = run_with_humidity_2017(H0_val, k_val, phi_val)
    if len(dates) == 0:
        return 1e15
...
def objective_function_unweighted_2017(params):
    """
    Standard (unweighted) objective function for the 2017 direct-start
